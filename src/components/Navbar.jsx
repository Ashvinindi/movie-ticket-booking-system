...
overflow-hidden transition-[width] duration-300 ${isOpen ? 'max-md:w-full': 'max-md:w-0'}`}>
      ...
      <X className='md:hidden w-10 h-6 cursor-pointer' onClick={()=> setIsOpen(!isOpen)}/>  
    ...
    <div className='flex items-center gap-8'>
      ...
      {
        !user ? (
          <button onClick={openSignIn} className='px-4 py-1 sm:px-7 sm:py-2 bg-[#F84565] hover:bg-[#D63854]
 transition rounded-full font-medium cursor-pointer'> Login</button>
        ) : (
          <UserButton> 
            <UserButton.MenuItems>
              <UserButton.Action lable="My Bookings" lableIcon=
